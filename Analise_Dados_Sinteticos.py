...
total_vendas_por_regiao = df_completo.groupby('Região')['Vendas'].sum().reset_index()
...
total_vendas = total_vendas_por_regiao['Vendas'].sum()
...
total_vendas_por_regiao['Vendas'] = total_vendas_por_regiao['Vendas'].round(2)
produtos_mais_vendidos['Quantidade'] = produtos_mais_vendidos['Quantidade'].round(2)

total_vendas_por_regiao.to_csv('total_vendas_por_regiao.csv', index=False)
